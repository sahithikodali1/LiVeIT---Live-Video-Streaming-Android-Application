...
class VideoClientApp(App):
    ...
    def start_stream(self, instance):
        ...
        self.server_ip = self.server_ip_input.text.strip()  # Get server IP from TextInput
    ...
    def update(self, dt):
        if self.streaming and self.camera.texture is not None:
            ...
            try:
                ...
                # Encode the frame using JPEG and then pickle the data
                ret, buffer = cv2.imencode(".jpg", mirrored_pixels, [int(cv2.IMWRITE_JPEG_QUALITY), 30])
                x_as_bytes = pickle.dumps(buffer)

                # Send the timestamp and frame to the server
                ...
                total_data_sent = len(x_as_bytes)
    ...
    def stop_stream(self, instance):
        if self.server_ip:
            ...
            print(f"Average Sent Data at client: {avg_data_sent} bytes")
